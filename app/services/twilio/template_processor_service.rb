class Twilio::TemplateProcessorService
  ...
  def convert_text_template(unixp_params)
    return process_key_value_params(unixp_params['processed_params']) if unixp_params['processed_params'].present?

    process_whatsapp_format_params(unixp_params['parameters'])
  end
  ...
  def convert_media_template(unixp_params)
    content_variables = {}

    # Handle processed_params format (key-value pairs)
    if unixp_params['processed_params'].present?
      unixp_params['processed_params'].each do |key, value|
        content_variables[key.to_s] = value.to_s
      end
    else
      # Handle parameters format (WhatsApp Cloud API format)
      parameter_index = 1
      unixp_params['parameters']&.each do |component|
